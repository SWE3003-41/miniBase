...
public class Parser {
    ...
    public static Predicate.Op getOp(String s) throws minibase.ParsingException {
        if (s.equals("="))
            ...
            if (isJoin) { // join node
                ...
                if (!op2const) { // right op is a nested query
                    try {
                        ...
                        throw new minibase.ParsingException("Invalid subquery "
                                + ops.elementAt(1));
    ...
    public LogicalPlan parseQueryLogicalPlan(TransactionId tid, ZQuery q)
            throws IOException, Zql.ParseException, minibase.ParsingException {
        @SuppressWarnings("unchecked")
        ...
        for (int i = 0; i < from.size(); i++) {
            ...
            try {
                ...
                e.printStackTrace();
                throw new minibase.ParsingException("Table "
            ...
            if (si.getAggregate() != null) {
                ...
                if (groupByField != null
                        && !(groupByField.equals(si.getTable() + "."
                                ...
                                .getColumn()))) {
                    throw new minibase.ParsingException("Non-aggregate field "
                            + si.getColumn()
    ...
    public Query handleQueryStatement(ZQuery s, TransactionId tId)
            ...
            minibase.ParsingException, Zql.ParseException {
        ...
        if (physicalPlan != null) {
            Class<?> c;
            try {
                c = Class.forName("minibase.OperatorCardinality");
                ...
                c = Class.forName("minibase.QueryPlanVisualizer");
                m = c.getMethod(
    ...
    public Query handleInsertStatement(ZInsert s, TransactionId tId)
        ...
        try {
            ...
            throw new minibase.ParsingException("Unknown table : "
                    + s.getTable());
        }
            ...
            for (ZExp e : values) {
                ...
                if (zc.getType() == ZConstant.NUMBER) {
                    if (td.getFieldType(i) != Type.INT_TYPE) {
                        throw new minibase.ParsingException("Value "
                                + zc.getValue()
    ...
    public void handleTransactStatement(ZTransactStmt s)
        ...
        if (s.getStmtType().equals("COMMIT")) {
            ...
            throw new minibase.ParsingException("Unsupported operation");
        }
    }
        ...
        try {
            ...
            if (s instanceof ZTransactStmt)
                ...
                try {
                    ...
                    if (a instanceof minibase.ParsingException
    ...
    protected void start(String[] argv) throws IOException {
        ...
        if (!interactive) {
            ...
            String line;
            boolean quit = false;
            while (!quit && (line = reader.readLine("MiniBase> ")) != null) {
